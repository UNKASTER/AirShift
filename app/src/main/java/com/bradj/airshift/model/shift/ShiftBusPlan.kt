 ...
 * 班车时刻与选车规则。
 *
 * 到位要求由用户确认：出港航班最晚提前 70 分钟到位，进港航班最晚提前 15 分钟到位，
 * 与 [com.bradj.airshift.model.DutyTimeline] 共用同一组提前量常量。
...
object ShiftBusPlan {
    ...
    val OUTBOUND_REPORT_LEAD_MINUTES: Int = DutyTimeline.DEPARTURE_GATE_ARRIVAL_LEAD_MINUTES.toInt()
    val INBOUND_REPORT_LEAD_MINUTES: Int = DutyTimeline.INBOUND_GATE_ARRIVAL_LEAD_MINUTES.toInt()
    ...
    // 清晨上岗的槽位首个任务是出港（到位提前 70 分钟），下午上岗的是过站进港（到位提前 15 分钟）。
